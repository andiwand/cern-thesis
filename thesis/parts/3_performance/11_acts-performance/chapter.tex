This chapter presents an extensive tracking performance study with the OpenDataDetector (ODD) using Acts and its Examples. Performance results are obtained for all algorithms of the tracking chain, from seeding to vertexing, with particular focus on track finding and fitting. The performance is evaluated using various metrics, including efficiency, fake rate, and resolutions and pulls of estimated parameters. The results are presented for different single particle samples ($\mu$, $\pi$, $e$) and for \ttbar events with different pileup scenarios. Reconstructed quantities are compared to the truth information to assess the quality of the reconstruction.
...
\begin{itemize}
    \item 100\% of the measurements forming a seed must be matched to the same truth particle
...
Figure \ref{fig:perf/odd/seeding-efficiency-mixed} shows the seeding efficiency for single $\mu$, $\pi$, and $e$ with $p_T$ = 10 GeV as a function of $\eta$. The observed efficiency is similar for $\pi$ and $\mu$, but generally lower for $e$. The efficiency for $e$ drops with increasing $|\eta|$. This can be explained by the increasing material $X/X_0$ towards higher $|\eta|$ values, shown in Figure \ref{fig:intro/acts/odd-material-x0-eta}.
...
Figure \ref{fig:perf/odd/finding-duplication} shows the track duplication for single $\mu$ with $p_T$ = 1 GeV as a function of $\eta$. The duplication is defined as the number of reconstructed tracks matched to the same truth particle. The observed duplication is well below 20\% over the entire $\eta$ range. Compared to the seed duplication in Figure \ref{fig:perf/odd/seeding-duplication}, the track duplication is significantly lower. This is expected, as the track finding algorithm is configured to remove duplicated seeds. The remaining duplicates will be handled by the ambiguity resolution algorithm, later in the reconstruction chain.
...
Figure \ref{fig:perf/odd/fitting-resolution-d0-eta-mu} shows the resolution of the transverse impact parameter $d_0$ as a function of $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV. The resolution is defined as the width of the residual distribution. The residual is defined as the difference between the fitted and simulated truth value $(x^\text{reco} - x^\text{true})$. The resolution is determined by the spatial resolution of the innermost layers, the amount of material travered by the particle, and its momentum. For high momentum paritcles the resolution converges to the intrinsic resolution of the detector, determined by the spatial resolution of the innermost layer.

\begin{figure}[h!]
    \centering
    \includegraphics[width=1.0\textwidth]{parts/3_performance/11_acts-performance/figures/fitting_resolution_z0_vs_pt_mixed.pdf}
    \caption{Track resolution of $z_0$ as a function of $p_T$ for single $\mu$, $\pi$, and $e$.}
    \label{fig:perf/odd/fitting-resolution-z0-pt-mixed}
\end{figure}

Figure \ref{fig:perf/odd/fitting-resolution-z0-pt-mixed} shows the resolution of the longitudinal impact parameter $z_0$ as a function of $p_T$ for single $\mu$, $\pi$, and $e$. The observed resolution is similar for all three particle types across the entire $p_T$ range. At high momentum the resolution converges to the intrinsic resolution of the detector.
...
Figure \ref{fig:perf/odd/fitting-pullmean-eta-mu} shows the mean of the pull distribution of all fitted track parameters at the interaction point as a function of $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV. The pull is defined as the residual divided by the estimated uncertainty of the fitted parameter $(\frac{x^\text{reco} - x^\text{true}}{\sigma_x^\text{reco}})$. The mean of the pull distribution should be 0 for an unbiased parameter estimator. The observed pull mean is close to 0 for all three $p_T$ values. For 100 GeV the pull mean depends on $\eta$ in the non-bending plane $z_0$ and $\theta$.
...
Figure \ref{fig:perf/odd/fitting-pullwidth-eta-mu} shows the width of the pull distribution of all fitted track parameters at the interaction point as a function of $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV. The width of the pull distribution should be 1 for an unbiased error estimator. The observed pull width is within 5\% to its optimal in the barrel region $|\eta| < 1.0$ for all three $p_T$ values and deviates further outwards with up to 20\%. For 1 and 10 GeV the pull width is generally higher than 1, while for 100 GeV it is lower than 1. This suggests suboptimal material handling or improper scaling of the initial covariance.
