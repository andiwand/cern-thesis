...
Figure \ref{fig:perf/odd/seeding-efficiency-mixed} shows the seeding efficiency for single $\mu$, $\pi$, and $e$ with $p_T$ = 10 GeV as a function of true $\eta$. The observed efficiency is similar for $\pi$ and $\mu$, but generally lower for $e$. The efficiency for $e$ drops with increasing $|\eta|$. This can be explained by the increasing material $X/X_0$ towards higher $|\eta|$ values, shown in Figure \ref{fig:intro/acts/odd-material-x0-eta}.
...
Figure \ref{fig:perf/odd/seeding-fake-ttbar} shows the seeding fake ratio for \ttbar events with different pileup $\langle \mu \rangle$ as a function of reconstructed $\eta$. The fake ratio is defined as the number of reconstructed seeds that are not matched to a single truth particle divided by the number of reconstructed seeds. The observed fake ratio increases with increasing pileup $\langle \mu \rangle$ and peaks at the already discussed problematic region at $|\eta| \approx 2$.
...
\begin{figure}[h!]
    ...
    \caption{Average track duplication as a function of true $\eta$ for single $\mu$ with $p_T = 1$ GeV.}
    \label{fig:perf/odd/finding-duplication}
\end{figure}

Figure \ref{fig:perf/odd/finding-duplication} shows the average track duplication for single $\mu$ with $p_T$ = 1 GeV as a function of true $\eta$. The duplication is defined as the number of reconstructed tracks matched to the same truth particle. The observed duplication is well below 20\% over the entire $\eta$ range. Compared to the seed duplication in Figure \ref{fig:perf/odd/seeding-duplication}, the track duplication is significantly lower. This is expected, as the track finding algorithm is configured to remove duplicated seeds. The remaining duplicates will be handled by the ambiguity resolution algorithm, later in the reconstruction chain.
...
\begin{figure}[h!]
    ...
    \caption{Average track duplication as a function of true $\eta$ for \ttbar events with different pileup $\langle \mu \rangle$. The transparent markers on the top show the track duplication before ambiguity resolution.}
