...
Simulated hits corresponding to energy deposits in sensitive detectors undergo a digitization process to mimic the behavior of detector electronics and to produce readout signals.
...
The following events have been simulated and form the basis of the performance study:
\begin{itemize}
...
Particles and hits are stored in ROOT \cite{root} files and later used as inputs for the reconstruction chain. This also allows switching the digitization method without running the event simulation again.

\section{Reconstruction chain and cuts}

The event reconstruction is implemented on top of existing components in ACTS Examples. The reconstruction chain consists of the following steps: truth-based measurement formation, spacepoint creation, seeding, track finding, ambiguity resolution, track refitting, and primary vertexing. The individual steps and their performance are described in the following sections. The simulation and reconstruction use the same magnetic field, material, and alignment conditions.

ACTS Examples provides different truth-based algorithms for track seeding, track finding, vertex seeding, and vertex finding. These algorithms allow evaluation of the performance of individual algorithms under ideal conditions. Truth-based algorithms were used in this study to compare the performance of the reconstruction chain with and without truth information.

Since the ODD provides time information in the pixel system, the reconstruction chain was designed to support 4D reconstruction wherever possible (see Chapter \ref{ch:dev/4d_tracking}). This includes spacepoint creation, parameter estimation, track finding, track fitting, vertex finding, and vertex fitting.

Before measuring the performance of the reconstruction chain, cuts are applied to select the particles and measurements used for performance evaluation. The following truth cuts are applied to the simulated particles:

\begin{itemize}
    \item true $p_T \ge 1$ GeV
    \item true $|\eta| < 3.0$
    \item true $q \ne 0$
...
\begin{itemize}
    ...
    \item outliers and/or holes in the track $\le 3$
\end{itemize}

After reconstruction, truth information is used to match reconstructed objects to truth particles. This allows evaluation of the performance of the reconstruction algorithms. The following cuts are applied to match reconstructed objects to truth:
...
Tracks can be shared between multiple reconstructed vertices. Track weight measures how well a track matches a vertex, relative to other vertices. A vertex with 70\% correctly assigned track weight is considered reconstructed \textit{cleanly}.
...
Figure \ref{fig:perf/odd/detector-material} shows the material validation up to the long strip system for $X/X_0$ as a function of $\eta$ between ACTS and Geant4. The observed material in ACTS agrees with Geant4 within 10\% for most of the $\eta$ range. There are some discrepancies at $|\eta| \approx 0.3$ and $|\eta| \approx 1.0$. These larger differences may be due to Geant4 steps that already reach into the solenoid.

\section{Seeding performance}

Seeding is the first step towards finding tracks after spacepoint formation. ACTS Examples offers different seeding algorithms, including truth-based seeding and triplet seeding. Truth-based seeders can be used to evaluate the performance of the reconstruction chain under optimal conditions. The triplet seeding is a more realistic approach and can be used in real experiments. The core idea of the triplet seeding algorithm is to process all reasonable combinations of three spacepoints and select those that may have been produced by a single particle.

It should be noted that the triplet seeding algorithm and its configuration are currently not optimized for the ODD. For this reason, it may produce low efficiency and high fake rates. Achieving optimal performance with the triplet seeding algorithm was not an objective of this study.
...
Figure \ref{fig:perf/odd/seeding-redundancy} shows the average seed redundancy for single $\mu$ with $p_T$ = 1 GeV as a function of true $\eta$. Redundancy is defined as the number of reconstructed seeds matched to the same truth particle, also called seed duplication. Generally, duplication should be avoided in tracking, and if it appears, it has to be removed at a later stage. In the case of the triplet seeding algorithm, duplication is expected and required for robustness against different detector conditions. Given the current configuration, redundancy is about 6 for $|\eta| < 1.5$ and drops at $|\eta| \approx 2$. This drop coincides with the drop in efficiency in Figure \ref{fig:perf/odd/seeding-efficiency-mu}, suggesting that the seeding configuration or detector layout is suboptimal in this region. In the forward region, at $|\eta| > 2.5$, redundancy increases to about 12 due to the number of endcap layers.
...
Figure \ref{fig:perf/odd/seeding-efficiency-mixed} shows the seeding efficiency for single $\mu$, $\pi$, and $e$ with $p_T$ = 10 GeV as a function of true $\eta$. The observed efficiency is similar for $\pi$ and $\mu$, but generally lower for $e$. The efficiency for $e$ drops with increasing $|\eta|$. This can be explained by increased material interactions (such as multiple scattering and bremsstrahlung) at higher $|\eta|$ values (see Figure \ref{fig:intro/acts/odd-material-x0-eta}).
...
Figure \ref{fig:perf/odd/seeding-fake-ttbar} shows the seeding fake ratio for \ttbar events with different pileup $\langle \mu \rangle$ as a function of reconstructed $\eta$. The fake ratio is defined as the number of reconstructed seeds not matched to a single truth particle divided by the total number of reconstructed seeds. The observed fake ratio increases with increasing pileup $\langle \mu \rangle$ and peaks at the already discussed problematic region $|\eta| \approx 2$.

\section{Track finding performance}

After seeding, the track finding algorithm is used to find complete tracks. The track finding algorithm in ACTS Examples is based on the combinatorial Kalman filter (CKF) (see Chapter \ref{ch:dev/track_finding}). The implementation of this algorithm has been reworked during the course of this thesis. These developments are described in detail in Chapter \ref{ch:dev/track_finding}. Briefly, the CKF starts propagation from the seed and collects measurements along the way. These measurements are used to iteratively refine the trajectory by updating the track parameters using the Kalman filter formalism. The track finding algorithm is designed not only to find all measurements, but also to provide an optimal fit to them. To achieve this, the CKF models material interactions accurately and has support for measurement calibration. Except for electrons, a refit should not be necessary after the track finding step.

The performance of the seeding is essential for the track finding performance. A particle's track can only be reconstructed if there is a seed for it. Seed deduplication can be used to avoid duplication of similar tracks.

Track finding performance is evaluated using the same metrics as seeding: efficiency, duplication, and fake ratio. Additionally, the fitting performance is evaluated with the resolution and pull of the fitted track parameters at the interaction point.
...
\begin{figure}[thb]
    ...
    \caption{Technical track finding efficiency as a function of true $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV.}
    \label{fig:perf/odd/finding-efficiency-mu}
\end{figure}

Figure \ref{fig:perf/odd/finding-efficiency-mu} shows the track finding efficiency for single $\mu$ as a function of true $\eta$ for different $p_T$ values. Technical efficiency is defined as the ratio of reconstructed particles with at least one matched track to the number of selected particles. Matching requires at least 50\% of the measurements on the track to come from the same truth particle, and at least 50\% of all the particle's measurements to be covered by the track. The observed efficiency for transverse momenta of 10 and 100 GeV is comparable to the seeding efficiency. For $p_T = 1$ GeV, the efficiency is significantly lower. This may be due to various reasons, such as an imprecise material map, incorrect implementation of energy loss or multiple scattering in ACTS, or a problem with navigation at low $p_T$. Increasing the $\chi^2$ cut for assigning measurements to the track improves efficiency. Multiple scattering dominates at lower $p_T$, and the fact that the original $\chi^2$ cut is sufficient for higher $p_T$ suggests that material handling is not optimal.
...
Figure \ref{fig:perf/odd/finding-duplication} shows the track duplication ratio for single $\mu$ with $p_T$ = 1 GeV as a function of reconstructed $\eta$. The duplication ratio is defined as the fraction between duplicated and reconstructed tracks. A track is classified as a duplicate if it matches an already matched truth particle. Observed duplication is below 17\% over the entire $\eta$ range. Compared to the average seed redundancy in Figure \ref{fig:perf/odd/seeding-redundancy}, the track duplication ratio is significantly lower. This is expected, as the track finding algorithm is configured to remove duplicated seeds. Duplication is still quite high, which suggests that track finding is not successful in recovering all measurements in some cases. The remaining duplicates will be handled by the ambiguity resolution algorithm later in the reconstruction chain.

\begin{figure}[thb]
    \centering
    \includegraphics[width=1.0\textwidth]{parts/3_performance/11_odd-performance/figures/finding_efficiency_mixed.pdf}
    \caption{Technical track finding efficiency as a function of true $\eta$ for single $\mu$, $\pi$, and $e$ with $p_T = 10$ GeV.}
    \label{fig:perf/odd/finding-efficiency-mixed}
\end{figure}

Figure \ref{fig:perf/odd/finding-efficiency-mixed} shows the track finding efficiency for single $\mu$, $\pi$, and $e$ with $p_T$ = 10 GeV as a function of true $\eta$. The observed efficiency for $\pi$ is up to 1\% lower than for $\mu$. Given that the $\pi$ must have left at least 6 measurements in the detector and at least 3 in the pixels, performance should be similar to that for $\mu$. The reason for this is under investigation. $\pi$ are subject to nuclear interactions and can get stopped while traversing the detector. The CKF branch stopper could cause inefficiencies due to missing measurements at the end of the track. This can happen if the seed does not start from the innermost measurement, or if intermediate measurements are not assigned to the track. For $e$, efficiency is significantly lower, with about 95\% over most of the $\eta$ range, but dropping to about 80\% at $|\eta| \approx 1.7$. The poor finding performance for $e$ can be explained by the track finding algorithm not being optimized for this case. Electrons are affected not only by multiple scattering and ionization losses, but also by bremsstrahlung.

To recover the remaining $e$ efficiency, the track finding algorithm must be robust against bremsstrahlung. This can be partially achieved by increasing the $\chi^2$ cut for assigning measurements to the track; however, this will also increase the fake rate. Ideally, track finding can be informed by a region-of-interest (ROI), for example provided by an electromagnetic calorimeter. Otherwise, the algorithm must assume that any seed could come from an electron, which would generally increase compute time and fake rate.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{parts/3_performance/11_odd-performance/figures/fitting_pullmean_vs_eta_mu.pdf}
...
Figure \ref{fig:perf/odd/fitting-pullmean-eta-mu} shows the mean of the pull distribution for all fitted track parameters at the interaction point as a function of true $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV. The pull is defined as the residual divided by the estimated uncertainty of the fitted parameter $(\frac{x^\text{reco} - x^\text{true}}{\sigma_x^\text{reco}})$. The mean of the pull distribution should be 0 for an unbiased parameter estimator. The observed pull mean is close to 0 for all three $p_T$ values. For 100 GeV the pull mean depends on $\eta$ in the non-bending plane $z_0$ and $\theta$.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{parts/3_performance/11_odd-performance/figures/fitting_pullwidth_vs_eta_mu.pdf}
...
Figure \ref{fig:perf/odd/fitting-pullwidth-eta-mu} shows the width of the pull distribution for all fitted track parameters at the interaction point as a function of true $\eta$ for single $\mu$ with $p_T = 1,10,100$ GeV. The width of the pull distribution should be 1 for an unbiased error estimator. The observed pull width is within 5\% to its optimal in the barrel region $|\eta| < 1.0$ for all three $p_T$ values and deviates further outwards with up to 20\%. For 1 and 10 GeV the pull width is generally higher than 1, while for 100 GeV it is lower than 1. This suggests suboptimal material handling or improper scaling of the initial covariance.
...
Figure \ref{fig:perf/odd/vertex-resolution} shows the residual distribution of the hard-scatter vertex for \ttbar events. The observed residual distribution centers at 0 for the spatial dimensions and fits well to a gaussian distribution. The $t$ residuals show a bias which is due to the pion particle hypothesis during reconstruction. This is caused by particles with significantly different masses from pions, like protons, since they produce a biased time of flight estimate. The resolution scale is smaller compared to the track ones, since the vertex fitter will combine the information from all tracks.
