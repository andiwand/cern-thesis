...
For track finding and fitting algorithms in ACTS the utilization of time information comes practically for free as they rely on the propagation, which does include time, and the measurements, which carry time information if the user provides it.
...
The same is true for the track fitting. The fitting algorithms in ACTS are generic and do not need to be adapted to make use of time information. The time information is automatically picked up from the measurements and used in the fit. The user only needs to be aware that the time parameter is part of the fit and can be used to cut on the track quality.
